...
func (p *parser) parseUnaryExprList(lhs bool) (list []ast.Expr) {
	if p.trace {
		defer un(trace(p, "UnaryExpressionList"))
	}

	list = append(list, p.checkExpr(p.parseUnaryExpr(lhs)))
	for p.tok == token.COMMA {
		p.next()
		list = append(list, p.checkExpr(p.parseUnaryExpr(lhs)))
	}
...
func (p *parser) parseTypeName() ast.Expr {
	...
	var retparams *ast.TypeParamsExpr
	if p.tok == token.LSS {
		pos := p.pos
		p.next()
	...
	if p.tok == token.PERIOD {
		...
		p.resolve(ident)
		sel := p.parseIdent()
		ret = &ast.SelectorExpr{X: ident, Sel: sel}
	} else {
		ret = ident
	}

	if retparams != nil {
		retparams.Type = ident
		ret = retparams
	}
...
func (p *parser) parseTypeSpec(doc *ast.CommentGroup, _ token.Token, _ int) ast.Spec {
	...
	if len(params) > 0 {
		return &ast.ParamedTypeSpec{Params: params, TypeSpec: *spec}
	}
